...
class Model(tf.Module):
    ...
    def predict(self, inputs):
        ...
        y0 = tf.reshape(inputs, [inputs.shape[0], -1])
        y1 = tf.nn.tanh(tf.add(tf.matmul(y0, self._W1), self._b1))
        y2 = tf.nn.softmax(tf.add(tf.matmul(y1, self._W2), self._b2))
        return y0, y1, y2
    ...
    def train_epoch(self, dataset):
        for batch in dataset.batches(self._args.batch_size):
            ...
            d, h, o = self.predict(batch["images"])
            ...
            d_2 = o - tf.one_hot(batch["labels"], o.shape[1])  # deriv. loss a softmax
            d_b2 = tf.reduce_mean(d_2, axis=0)
            d_W2 = tf.reduce_mean(tf.einsum("ai,aj->aij", h, d_2), axis=0)

            d_1 = tf.subtract(tf.constant(1.0), tf.square(h)) * (d_2 @ tf.transpose(self._W2))
            d_b1 = tf.reduce_mean(d_1, axis=0)
            d_W1 = tf.reduce_mean(tf.einsum("ai,aj->aij", d, d_1), axis=0)
            ...
            self._W1.assign_sub(self._args.learning_rate * d_W1)
            self._b1.assign_sub(self._args.learning_rate * d_b1)
            self._W2.assign_sub(self._args.learning_rate * d_W2)
            self._b2.assign_sub(self._args.learning_rate * d_b2)
