...
def main(args):
    ...
    optimizer_class = getattr(tf.optimizers, args.optimizer)
    optargs = {}
    if args.momentum:
        optargs["momentum"] = args.momentum
    if args.decay:
        batches = args.epochs * mnist.train.size // args.batch_size
        if args.decay == "exponential":
            optargs["learning_rate"] = tf.optimizers.schedules.ExponentialDecay(
                args.learning_rate, batches, args.learning_rate_final / args.learning_rate)
        elif args.decay == "polynomial":
            optargs["learning_rate"] = tf.optimizers.schedules.PolynomialDecay(
                args.learning_rate, batches, args.learning_rate_final)
    else:
        optargs["learning_rate"] = args.learning_rate

    model.compile(
        optimizer=optimizer_class(**optargs),
    ...
    if args.decay:
        print("Final learning rate: {:g}".format(model.optimizer.learning_rate(model.optimizer.iterations)))
