...
class Model(tf.Module):
    ...
    def predict(self, inputs):
        ...
        hidden = tf.reshape(inputs, [inputs.shape[0], -1])
        hidden = hidden @ self._W1 + self._b1
        hidden = tf.nn.tanh(hidden)
        hidden = hidden @ self._W2 + self._b2
    ...
    def train_epoch(self, dataset):
        for batch in dataset.batches(self._args.batch_size):
            ...
            with tf.GradientTape() as tape:
                ...
                if True:
                    # Compute the loss using one-hot encoded labels
                    loss = tf.math.reduce_mean(tf.math.reduce_sum(-tf.one_hot(batch["labels"], MNIST.LABELS) * tf.math.log(probabilities), axis=1))
                else:
                    # The loss can be also computed by indexing the predicted probabilities
                    loss = tf.math.reduce_mean(-tf.math.log(tf.gather(probabilities, tf.cast(batch["labels"], tf.int32), batch_dims=1)))
