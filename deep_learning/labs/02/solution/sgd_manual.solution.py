...
class Model(tf.Module):
    ...
    def predict(self, inputs):
        ...
        probabilities = hidden @ self._W2 + self._b2
    ...
    def train_epoch(self, dataset):
        for batch in dataset.batches(self._args.batch_size):
            ...
            inputs, hidden, probabilities = self.predict(batch["images"])
            ...
            d_logits = probabilities - tf.one_hot(batch["labels"], MNIST.LABELS)
            d_hidden = tf.linalg.matvec(self._W2, d_logits)
            d_hidden = d_hidden * (1 - hidden * hidden)
            ...
            self._b2.assign_sub(self._args.learning_rate * tf.math.reduce_mean(d_logits, axis=0))
            self._W2.assign_sub(self._args.learning_rate * tf.math.reduce_mean(hidden[:, :, tf.newaxis] * d_logits[:, tf.newaxis, :], axis=0))
            self._b1.assign_sub(self._args.learning_rate * tf.math.reduce_mean(d_hidden, axis=0))
            self._W1.assign_sub(self._args.learning_rate * tf.math.reduce_mean(inputs[:, :, tf.newaxis] * d_hidden[:, tf.newaxis, :], axis=0))
    ...
    def evaluate(self, dataset):
        ...
        for batch in dataset.batches(self._args.batch_size):
            ...
            correct += tf.math.reduce_sum(tf.cast(predictions == batch["labels"], tf.int32))
