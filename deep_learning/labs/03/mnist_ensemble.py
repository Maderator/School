...
def main(args):
    ...
    for model in range(args.models):
        ...
        if model != 0:
            inp = tf.keras.layers.Input(shape=[MNIST.H, MNIST.W, MNIST.C])
            avg = tf.keras.layers.Average()([m(inp) for m in models[0:model+1]])
            m = tf.keras.models.Model(inputs=inp, outputs=avg)
            m.compile(
                optimizer=tf.optimizers.Adam(),
                loss=tf.losses.SparseCategoricalCrossentropy(),
                metrics=[tf.metrics.SparseCategoricalAccuracy(name="accuracy")],
            )
            _, ensemble_accuracy = m.evaluate(mnist.dev.data["images"], mnist.dev.data["labels"])
        else:
