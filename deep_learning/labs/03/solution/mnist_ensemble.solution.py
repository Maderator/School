...
parser.add_argument("--method", default="keras", type=str, help="Method type (keras/manual).")
...
def main(args):
    ...
    for model in range(args.models):
        ...
        if args.method == "keras":
            ensemble_inputs = tf.keras.layers.Input(shape=[MNIST.H, MNIST.W, MNIST.C])
            ensemble_models = [model(ensemble_inputs) for model in models[:model + 1]]
            ensemble_outputs = tf.keras.layers.average(ensemble_models) if model > 0 else ensemble_models[0]
            ensemble_model = tf.keras.Model(inputs=ensemble_inputs, outputs=ensemble_outputs)
            ensemble_model.compile(metrics=[tf.metrics.SparseCategoricalAccuracy(name="accuracy")])
            ...
            ensemble = ensemble + predictions if model > 0 else predictions
            ensemble_accuracy = tf.metrics.SparseCategoricalAccuracy()(mnist.dev.data["labels"], ensemble)
