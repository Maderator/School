...
class Network(tf.keras.Model):
    def __init__(self, args):
        ...
        def create_layer(hidden, params):
            if params[0] == "C":
                hidden = tf.keras.layers.Conv2D(int(params[1]), kernel_size=int(params[2]), strides=int(params[3]), padding=params[4], activation="relu")(hidden)
            elif params[0] == "CB":
                hidden = tf.keras.layers.Conv2D(int(params[1]), kernel_size=int(params[2]), strides=int(params[3]), padding=params[4], use_bias=False, activation=None)(hidden)
                hidden = tf.keras.layers.BatchNormalization()(hidden)
                hidden = tf.keras.activations.relu(hidden)
            elif params[0] == "M":
                hidden = tf.keras.layers.MaxPooling2D(pool_size=int(params[1]), strides=int(params[2]))(hidden)
            elif params[0] == "F":
                hidden = tf.keras.layers.Flatten()(hidden)
            elif params[0] == "H":
                hidden = tf.keras.layers.Dense(int(params[1]), activation=tf.nn.relu)(hidden)
            elif params[0] == "D":
                hidden = tf.keras.layers.Dropout(float(params[1]))(hidden)
            else:
        ...
        # parse layers description, handle/replace inner R-layers
        args_cnn = ""
        args_cnn_prev = args.cnn
        while True:
            args_cnn = re.sub(r"\[(.+),(.+)\]", r"[\1|\2]", args_cnn_prev)
            if args_cnn == args_cnn_prev:
                break
            args_cnn_prev = args_cnn

        # for each layer...
        layers_def = args_cnn.split(",")
        for ld in layers_def:
            # split parameters
            lp = re.split("-", ld)
            print(lp)
            ...
            if lp[0] == "R":
                # prepare parameters
                rp = "-".join(lp[1:])[1:-1].split("|")
                # prepare input
                hidden_r = hidden
                # for each layer in R-[*]
                for ld_inner in rp:
                    # split parameters
                    ld_inner_p = re.split("-", ld_inner)
                    print("  - ", ld_inner_p)
                    # create layer and connect it
                    hidden_r = create_layer(hidden_r, ld_inner_p)
                # add outputs of R-[] block to the input and connect it
                hidden = tf.keras.layers.Add()([hidden, hidden_r])
            else:
                # create layer
                hidden = create_layer(hidden, lp)
