...
class Network(tf.keras.Model):
    def __init__(self, args):
        ...
        hidden = inputs
        for layer in args.cnn.replace("-[", ",").replace("]", ",]").split(","):
            ...
            if name == "C":
                ...
                hidden = tf.keras.layers.Conv2D(filters, kernel_size, stride, padding, activation=tf.nn.relu)(hidden)
            if name == "CB":
                filters, kernel_size, stride, padding = params
                hidden = tf.keras.layers.Conv2D(filters, kernel_size, stride, padding, use_bias=False)(hidden)
                hidden = tf.keras.layers.BatchNormalization()(hidden)
                hidden = tf.keras.layers.ReLU()(hidden)
            if name == "M":
                kernel_size, stride = params
                hidden = tf.keras.layers.MaxPooling2D(kernel_size, stride)(hidden)
            if name == "R":
                residual = hidden
            if name == "]":
            ...
            if name == "D":
                dropout_rate, = params
                hidden = tf.keras.layers.Dropout(float(dropout_rate))(hidden)
