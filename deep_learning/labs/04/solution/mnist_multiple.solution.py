...
class Network(tf.keras.Model):
    def __init__(self, args):
        ...
        layers = [
            tf.keras.layers.Conv2D(10, 3, 2, "valid", activation=tf.nn.relu),
            tf.keras.layers.Conv2D(20, 3, 2, "valid", activation=tf.nn.relu),
        ...
        #   two outputs.
        direct_prediction = tf.keras.layers.Concatenate()(hiddens)
        direct_prediction = tf.keras.layers.Dense(200, activation=tf.nn.relu)(direct_prediction)
        direct_prediction = tf.keras.layers.Dense(1, activation=tf.nn.sigmoid)(direct_prediction)

        label_prediction_layer = tf.keras.layers.Dense(MNIST.LABELS, activation=tf.nn.softmax)
        labels_prediction = [label_prediction_layer(hidden) for hidden in hiddens]

        indirect_prediction = tf.argmax(labels_prediction[0], axis=-1) > tf.argmax(labels_prediction[1], axis=-1)

        outputs = {
            "direct_prediction": direct_prediction,
            "digit_1": labels_prediction[0],
            "digit_2": labels_prediction[1],
