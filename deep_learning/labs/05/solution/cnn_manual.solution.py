...
parser.add_argument("--use_reference", default=False, action="store_true", help="Use reference implementation.")

class Convolution:
    def __init__(self, channels, kernel_size, stride, input_shape, use_reference):
        ...
        if self._use_reference:
            self._conv = tf.keras.layers.Conv2D(channels, kernel_size, stride, activation=tf.nn.relu, padding="valid", kernel_initializer=tf.initializers.GlorotUniform(seed=42), bias_initializer=tf.initializers.Zeros())
            self._conv.build([None] + input_shape)
    ...
    def forward(self, inputs):
        ...
        for y in range(self._kernel_size):
            for x in range(self._kernel_size):
                output += inputs[:,
                                 y:inputs.shape[1] - (self._kernel_size-1) + y:self._stride,
                                 x:inputs.shape[2] - (self._kernel_size-1) + x:self._stride] @ self._kernel[y, x]
        output += self._bias
    ...
    def backward(self, inputs, outputs, outputs_gradient):
        ...
        if self._use_reference:
            ...
            inputs_gradient, kernel_gradient, bias_gradient = tape.gradient(
                outputs, [inputs, self._conv.kernel, self._conv.bias], outputs_gradient)
            return inputs_gradient, [self._conv.kernel, self._conv.bias], [kernel_gradient, bias_gradient]

        outputs_gradient *= tf.cast(outputs > 0, tf.float32)

        bias_gradient = tf.math.reduce_sum(outputs_gradient, axis=[0,1,2])

        kernel_gradient = []
        for y in range(self._kernel_size):
            kernel_gradient.append([])
            for x in range(self._kernel_size):
                kernel_gradient[-1].append(tf.math.reduce_sum(
                    tf.expand_dims(outputs_gradient, axis=3) *
                    tf.expand_dims(inputs[:,
                                          y:inputs.shape[1] - (self._kernel_size-1) + y:self._stride,
                                          x:inputs.shape[2] - (self._kernel_size-1) + x:self._stride], axis=4), axis=[0, 1, 2]))
            kernel_gradient[-1] = tf.stack(kernel_gradient[-1], axis=0)
        kernel_gradient = tf.stack(kernel_gradient, axis=0)

        expanded_gradients = outputs_gradient
        if self._stride > 1:
            if True:
                # Expand the gradients using `tf.pad`
                expanded_gradients = tf.transpose(expanded_gradients, [0, 2, 3, 1])
                expanded_gradients = tf.reshape(tf.pad(tf.expand_dims(expanded_gradients, axis=4), [[0, 0]] * 4 + [[0, self._stride - 1]]), list(expanded_gradients.shape[:-1]) + [-1])
                expanded_gradients = tf.transpose(expanded_gradients, [0, 3, 2, 1])
                expanded_gradients = tf.reshape(tf.pad(tf.expand_dims(expanded_gradients, axis=4), [[0, 0]] * 4 + [[0, self._stride - 1]]), list(expanded_gradients.shape[:-1]) + [-1])
                expanded_gradients = tf.transpose(expanded_gradients, [0, 1, 3, 2])
            else:
                # Alternative expansion using tf.repeat and masking.
                # Start by generating a mask
                mask = tf.constant([1.] + [0] * (self._stride - 1))
                mask = tf.tile(mask, [(inputs.shape[2] - self._kernel_size + self._stride) // self._stride])
                mask = tf.stack([mask] + [tf.zeros_like(mask)] * (self._stride - 1), axis=0)
                mask = tf.tile(mask, [(inputs.shape[1] - self._kernel_size + self._stride) // self._stride, 1])
                mask = tf.expand_dims(mask, axis=2)
                # Repeat gradients and apply mask
                expanded_gradients = tf.repeat(tf.repeat(expanded_gradients, self._stride, axis=1), self._stride, axis=2) * mask
        expanded_gradients = tf.pad(expanded_gradients, [[0, 0], [self._kernel_size - 1, self._kernel_size - 1], [self._kernel_size - 1, self._kernel_size - 1], [0, 0]])

        inputs_gradient = 0
        for y in range(self._kernel_size):
            for x in range(self._kernel_size):
                inputs_gradient += expanded_gradients[:,
                                                      self._kernel_size - 1 - y:self._kernel_size - 1 - y + inputs.shape[1],
                                                      self._kernel_size - 1 - x:self._kernel_size - 1 - x + inputs.shape[2]
                                                      ] @ tf.transpose(self._kernel[y, x])
...
class Network:
    def __init__(self, args):
        ...
        for layer in args.cnn.split(","):
            ...
            self._convolutions.append(Convolution(channels, kernel_size, stride, input_shape, args.use_reference))
