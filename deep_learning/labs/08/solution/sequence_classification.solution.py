...
parser.add_argument("--cells_only", default=True, action="store_true", help="Use RNN cells only.")
...
class Network(tf.keras.Model):
    def __init__(self, args):
        ...
        if not args.cells_only:
            hidden = getattr(tf.keras.layers, args.rnn_cell)(args.rnn_cell_dim, return_sequences=True)(sequences)
        else:
            hidden = tf.keras.layers.RNN(getattr(tf.keras.layers, args.rnn_cell + "Cell")(args.rnn_cell_dim), return_sequences=True)(sequences)
