...
class Network(tf.keras.Model):
    ...
    def __init__(self, args, train):
        ...
        hidden = train.forms.word_mapping(words)
        ...
        if True:
            hidden = self.MaskElements(args.word_masking)(hidden)
        else:
            mask = tf.keras.layers.Dropout(args.word_masking)(tf.ones_like(hidden, dtype=tf.float32))
            hidden = hidden * tf.cast(mask != 0, tf.int64)
        ...
        hidden = tf.keras.layers.Embedding(train.forms.word_mapping.vocab_size(), args.we_dim)(hidden)
        ...
        unique_words, unique_words_indices = tf.unique(words.values)
        ...
        letters = tf.strings.unicode_split(unique_words, "UTF-8")

        # TODO: Map the letters into ids by using `char_mapping` of `train.forms`.
        letters = train.forms.char_mapping(letters)

        # TODO: Embed the input characters with dimensionality `args.cle_dim`.
        letters = tf.keras.layers.Embedding(train.forms.char_mapping.vocab_size(), args.cle_dim)(letters)
        ...
        cle = tf.keras.layers.Bidirectional(tf.keras.layers.GRU(args.cle_dim), merge_mode="concat")(
            letters.to_tensor(), mask=tf.sequence_mask(letters.row_lengths()))
        ...
        cle = tf.gather(cle, unique_words_indices)
        ...
        hidden = tf.RaggedTensor.from_tensor(
            tf.keras.layers.Bidirectional(rnn_layer, merge_mode="sum")(hidden.to_tensor(), mask=tf.sequence_mask(hidden.row_lengths())),
            hidden.row_lengths(),
        )
        ...
        predictions = tf.keras.layers.TimeDistributed(
            tf.keras.layers.Dense(train.tags.word_mapping.vocab_size(), activation=tf.nn.softmax))(hidden)
