...
class Network(tf.keras.Model):
    def __init__(self, args, train):
        ...
        self.target_mapping_size = self.target_mapping.vocab_size()

        # TODO(lemmatizer_noattn): Define
        # - `self.source_embedding` as an embedding layer of source chars into `args.cle_dim` dimensions
        source_vocab_size = self.source_mapping.vocab_size()
        self.source_embedding = tf.keras.layers.Embedding(source_vocab_size ,args.cle_dim)


        # TODO: Define
        # - `self.source_rnn` as a bidirectional GRU with `args.rnn_dim` units, returning **whole sequences**,
        #   summing opposite directions
        gru_layer = tf.keras.layers.GRU(units=args.rnn_dim, return_sequences=True) # return_sequences=False is default
        ...
        target_vocab_size = self.target_mapping.vocab_size()
        self.target_embedding = tf.keras.layers.Embedding(target_vocab_size, args.cle_dim)
        self.target_rnn_cell = tf.keras.layers.GRUCell(units=args.rnn_dim)
        self.target_output_layer = tf.keras.layers.Dense(target_vocab_size, activation=None)
    ...
    class DecoderTraining(tfa.seq2seq.BaseDecoder):
        ...
        def output_size(self):
            ...
            return tf.TensorShape(self.lemmatizer.target_mapping_size)
        @property
        ...
        def with_attention(self, inputs, states):
            ...
            attention_state_l_out = self.lemmatizer.attention_state_layer(states)
            attention_state_l_out = tf.expand_dims(attention_state_l_out, axis=1)
            state_source_sum = attention_state_l_out + self.attention_source_l_out

            tanh_out = tf.tanh(state_source_sum)
            attention_wl_out = self.lemmatizer.attention_weight_layer(tanh_out)
            attention_weights = tf.nn.softmax(attention_wl_out, axis=1)
            mult_out = tf.multiply(self.source_states, attention_weights)
            attention = tf.reduce_sum(mult_out, axis=1)
        ...
        def initialize(self, layer_inputs, initial_state=None, mask=None):
            ...
            self.attention_source_l_out = self.lemmatizer.attention_source_layer(self.source_states)

            # TODO(lemmatizer_noattn): Define `finished` as a vector of self.batch_size of `False` [see tf.fill].
            finished = tf.fill([self.batch_size], tf.constant(False, tf.bool))
            ...
            bows = tf.fill([self.batch_size], tf.constant(MorphoDataset.Factor.BOW, tf.int64))
            inputs = self.lemmatizer.target_embedding(bows)
        ...
        def step(self, time, inputs, states, training):
            ...
            finished = tf.equal(self.targets[:, time], tf.constant(MorphoDataset.Factor.EOW, tf.int64))
    ...
    class DecoderPrediction(DecoderTraining):
        ...
        def step(self, time, inputs, states, training):
            ...
            finished = tf.equal(outputs, tf.constant(MorphoDataset.Factor.EOW, tf.int32))
    ...
    def call(self, inputs, targets=None):
        ...
        if targets is not None:
            ...
            output, _, output_lens = decoder_training([source_states, target_charseqs])
        else:
            ...
            decoder_prediction = self.DecoderPrediction(self, maximum_iterations=tf.cast(source_charseqs.bounding_shape(1) + 10, tf.int32))
            output, _, output_lens = decoder_prediction(source_states)
