...
parser.add_argument("--clip", default=0.0, type=float, help="Clip gradient")
parser.add_argument("--dropout", default=0.0, type=float, help="Dropout")
parser.add_argument("--epochs", default=30, type=int, help="Number of epochs.")
parser.add_argument("--sentence_rnn_layers", default=2, type=int, help="Sentence RNN layers.")
parser.add_argument("--rnn_dim", default=256, type=int, help="RNN cell dimension.")
parser.add_argument("--seed", default=42, type=int, help="Random seed.")
parser.add_argument("--threads", default=1, type=int, help="Maximum number of threads to use.")
parser.add_argument("--word_masking", default=0.0, type=float, help="Word masking")

class Network(tf.keras.Model):
    def __init__(self, args, train, epoch_batches):
        super().__init__()
        ...
        self.source_embedding = tf.keras.layers.Embedding(self.source_mapping.vocab_size(), args.rnn_dim)
        ...
        self.target_embedding = tf.keras.layers.Embedding(self.target_mapping.vocab_size(), args.rnn_dim)
        ...
        # Sentence-level contextualization layers
        self.sentence_word_mapping = train.forms.word_mapping
        self.sentence_we_embedding = tf.keras.layers.Embedding(train.forms.word_mapping.vocab_size(), args.rnn_dim)
        self.sentence_lstms = [tf.keras.layers.Bidirectional(tf.keras.layers.LSTM(args.rnn_dim, return_sequences=True), merge_mode="sum")
                               for _ in range(args.sentence_rnn_layers)]
        self.dropout = tf.keras.layers.Dropout(args.dropout)
        self.word_masking = self.MaskElements(args.word_masking)

        # Compile the model
        self.compile(
            optimizer=tf.optimizers.Adam(
                learning_rate=tf.keras.experimental.CosineDecay(1e-3, args.epochs * epoch_batches),
                global_clipnorm=args.clip or None,
            ),
    ...
    class MaskElements(tf.keras.layers.Layer):
        ...
        def call(self, inputs, training):
            if training:
                return inputs.with_values(inputs.values * tf.cast(
                    tf.random.uniform(tf.shape(inputs.values)) >= self._rate, inputs.dtype))
            else:
                return inputs
    ...
    class DecoderTraining(tfa.seq2seq.BaseDecoder):
        ...
        def with_attention(self, inputs, states):
            ...
            projected = self.source_states_projected + tf.expand_dims(self.lemmatizer.attention_state_layer(states), axis=1)
            ...
            weights = tf.nn.softmax(self.lemmatizer.attention_weight_layer(tf.tanh(projected)), axis=1)
        ...
        def initialize(self, layer_inputs, initial_state=None, mask=None):
            self.source_states, inits, self.targets = layer_inputs
            self.source_states_projected = self.lemmatizer.attention_source_layer(self.source_states)
        ...
        def step(self, time, inputs, states, training):
            ...
            outputs = self.lemmatizer.dropout(outputs)
    ...
    def call(self, inputs, targets=None):
        ...
        source_embedded = self.dropout(self.source_embedding(source_charseqs))
        ...
        source_states = self.dropout(self.source_rnn(source_embedded.to_tensor(), mask=tf.sequence_mask(source_embedded.row_lengths())))

        # Sentence-level contextualization
        sentence_hidden = self.sentence_word_mapping(inputs)
        sentence_hidden = self.word_masking(sentence_hidden)
        sentence_hidden = self.dropout(self.sentence_we_embedding(sentence_hidden))
        sentence_hidden += inputs.with_values(source_states[:, 0])

        sentence_hidden = sentence_hidden.to_tensor()
        sentence_mask = tf.sequence_mask(inputs.row_lengths())
        for i, lstm in enumerate(self.sentence_lstms):
            output = self.dropout(lstm(sentence_hidden, mask=sentence_mask))
            sentence_hidden = output + (sentence_hidden if i else 0)
        source_inits = tf.boolean_mask(sentence_hidden, mask=sentence_mask)
        ...
        if targets is not None:
            ...
            output, _, output_lens = self.DecoderTraining(self)([source_states, source_inits, target_charseqs])
        else:
            ...
            output, _, output_lens = self.DecoderPrediction(
                self, maximum_iterations=tf.cast(source_charseqs.bounding_shape(1) + 10, tf.int32))([source_states, source_inits])
            output_lens -= 1
...
def main(args):
    ...
    model = Network(args, morpho.train, len(train))
