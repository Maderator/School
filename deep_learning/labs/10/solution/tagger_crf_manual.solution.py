...
class Network(tf.keras.Model):
    def __init__(self, args, train):
        ...
        predictions = tf.keras.layers.TimeDistributed(tf.keras.layers.Dense(train.tags.word_mapping.vocab_size()))(hidden)
        ...
        self._crf_weights = self.add_weight(
            name="crf_weights", shape=[train.tags.word_mapping.vocab_size()] * 2, dtype=tf.float32,
    ...
    def crf_loss(self, gold_labels, logits):
        ...
        class CRFCell(tf.keras.layers.AbstractRNNCell):
            ...
            def state_size(self):
                return logits.shape[-1]
            def call(this, inputs, states):
                alphas = inputs + tf.math.reduce_logsumexp(states[0][:, :, tf.newaxis] + self._crf_weights, axis=1)
                return alphas, [alphas]

        alphas = tf.keras.layers.RNN(CRFCell())(
            logits[:, 1:], initial_state=logits[:, :1].to_tensor()[:, 0])

        gold_pairs = tf.stack([gold_labels[:, :-1], gold_labels[:, 1:]], axis=-1)

        return -tf.math.reduce_mean(
            tf.math.reduce_sum(tf.gather(logits, gold_labels, batch_dims=2), axis=1)
            + tf.math.reduce_sum(tf.gather_nd(self._crf_weights, gold_pairs), axis=1)
            - tf.math.reduce_logsumexp(alphas, axis=-1)
        )
