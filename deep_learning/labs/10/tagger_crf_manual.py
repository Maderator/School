...
class Network(tf.keras.Model):
    def __init__(self, args, train):
        ...
        rnn_output = rnn_layer(embedded_words.to_tensor(), mask=tf.sequence_mask(embedded_words.row_lengths()))

        rnn_output = tf.RaggedTensor.from_tensor(rnn_output, embedded_words.row_lengths())
        ...
        classification_layer = tf.keras.layers.Dense(tags_size, activation=None)
    ...
    def _crf_logsumexp(self, inputs, sequence_lengths, transition_params):
        inputs_d = inputs.to_tensor()
        first_input = inputs_d[:, 0]
        rest_inputs = inputs[:, 1:]
        def scan_fn(fi, ri):
            new_a = ri + tf.reduce_logsumexp(fi + transition_params)
            return new_a

        alphas = tf.scan(scan_fn, elems=rest_inputs, initializer=first_input)
        all_alphas = tf.concat([tf.expand_dims(first_input, 1), alphas], 1)
        return all_alphas
    ...
    def crf_loss(self, gold_labels, logits):
        ...
        # COMPUTE SCORES
        unary_logfactor = tf.reduce_sum(tf.gather(logits, gold_labels, batch_dims=2), axis=1)
        
        begin_labels = gold_labels[:,:-1]
        end_labels = gold_labels[:,1:]
        pair_labels = tf.stack([begin_labels, end_labels], axis=2)
        pairwise_logfactor = tf.gather_nd(self._crf_weights, pair_labels)
        pairwise_logfactor = tf.reduce_sum(pairwise_logfactor, axis=1)
        scores = unary_logfactor + pairwise_logfactor

        # COMPUTE ALPHAS
        inputs_d = logits.to_tensor()
        first_input = inputs_d[:, 0, :]
        rest_inputs = inputs_d[:, 1:, :]
        rest_inputs = tf.transpose(rest_inputs, [1,0,2]) # so that we go (scan) through second dimension

        alphas = tf.scan(
            lambda fi, ri: ri + tf.reduce_logsumexp(
                tf.expand_dims(fi, 2) + 
                tf.expand_dims(self._crf_weights, 0), axis=1),
            elems=rest_inputs, initializer=first_input)
        
        alphas = tf.transpose(alphas, [1,0,2])
        all_alphas = tf.concat([tf.expand_dims(first_input, 1), alphas], axis=1)

        # COMPUTE LOGSUMEXP with alphas
        sequence_lengths = tf.cast(gold_labels.row_lengths(), tf.int32)
        idxs = tf.stack([tf.range(tf.shape(sequence_lengths)[0]), sequence_lengths-1], axis=1)
        ragged_part_alphas = tf.gather_nd(all_alphas, idxs)
        logsumexp = tf.reduce_logsumexp(ragged_part_alphas, axis=1)

        log_likelihood = scores - logsumexp

        return tf.math.reduce_mean(-log_likelihood, axis=0)
