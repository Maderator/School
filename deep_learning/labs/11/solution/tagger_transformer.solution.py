...
class Network(tf.keras.Model):
    ...
    class SelfAttention(tf.keras.layers.Layer):
        ...
        def call(self, inputs, mask):
            ...
            if True:
                ...
                attention_weights = attention_weights * mask - 1e9 * (1 - mask)
                attention = tf.nn.softmax(attention_weights, axis=-1)
    ...
    class Transformer(tf.keras.layers.Layer):
        def __init__(self, layers, dim, expansion, heads, dropout, *args, **kwargs):
            ...
            for _ in range(layers):
                for layer in [Network.SelfAttention(dim, heads), Network.FFN(dim, expansion)]:
                    self.sublayers.append((layer, tf.keras.layers.LayerNormalization()))
        ...
        def call(self, inputs, mask):
            ...
            pos_embeddings = (
                tf.range(tf.shape(inputs)[1], dtype=tf.float32)[:, tf.newaxis] /
                (10000 ** tf.linspace(0., 1., inputs.shape[2] // 2 + 1)[:-1])
            )
            pos_embeddings = tf.concat([tf.math.sin(pos_embeddings), tf.math.cos(pos_embeddings)], axis=-1)
            ...
            hidden = inputs + pos_embeddings
            for layer, layer_normalization in self.sublayers:
                hidden = layer_normalization(hidden + self.layer_dropout(
                    layer(hidden, mask=mask) if isinstance(layer, Network.SelfAttention) else layer(hidden)))
            return hidden
    ...
    def __init__(self, args, train):
        ...
        hidden = tf.RaggedTensor.from_tensor(
            Network.Transformer(layers=args.transformer_layers, dim=args.we_dim, expansion=args.transformer_expansion,
                                heads=args.transformer_heads, dropout=args.transformer_dropout)(
