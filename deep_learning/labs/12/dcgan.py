...
class GAN(tf.keras.Model):
    def __init__(self, args):
        ...
        x = tf.keras.layers.Dense(1024, activation=None, use_bias=False)(inputs) 
        x = tf.keras.layers.BatchNormalization()(x)
        x = tf.keras.layers.Activation("relu")(x)
        x = tf.keras.layers.Dense(MNIST.H // 4 * MNIST.W // 4 * 64, activation=None, use_bias=False)(x)
        x = tf.keras.layers.BatchNormalization()(x)
        x = tf.keras.layers.Activation("relu")(x)
        x = tf.keras.layers.Reshape(target_shape=[MNIST.H // 4, MNIST.W //4, 64])(x)
        x = tf.keras.layers.Conv2DTranspose(filters=32, kernel_size=5, strides=2, padding="same", activation=None, use_bias=False)(x)
        x = tf.keras.layers.BatchNormalization()(x)
        x = tf.keras.layers.Activation("relu")(x)
        outputs = tf.keras.layers.Conv2DTranspose(filters=1, kernel_size=5, strides=2, padding="same", activation="sigmoid")(x)
        self.generator = tf.keras.Model(inputs, outputs)
        ...
        x = tf.keras.layers.Conv2D(32, 5, padding="same", activation=None, use_bias=False)(inputs)
        x = tf.keras.layers.BatchNormalization()(x)
        x = tf.keras.layers.Activation("relu")(x)
        x = tf.keras.layers.MaxPool2D(pool_size=2, strides=2)(x)
        x = tf.keras.layers.Conv2D(64, 5, padding="same", activation=None, use_bias=False)(x)
        x = tf.keras.layers.BatchNormalization()(x)
        x = tf.keras.layers.Activation("relu")(x)
        x = tf.keras.layers.MaxPool2D(pool_size=2, strides=2)(x)
    ...
    def train_step(self, images):
        ...
        with tf.GradientTape() as tape:
            random_samples = self._z_prior.sample(sample_shape=tf.shape(images)[0], seed=self._seed)
            generated_images = self.generator(random_samples, training=True)
            out_discriminator = self.discriminator(generated_images, training=True)
            generator_loss = self.compiled_loss(y_true=tf.ones_like(out_discriminator), y_pred=out_discriminator)
        ...
        with tf.GradientTape() as tape2:
            ...
            real_loss = self.compiled_loss(y_true=tf.ones_like(discriminated_real), y_pred=discriminated_real)
            fake_loss = self.compiled_loss(y_true=tf.zeros_like(discriminated_fake), y_pred=discriminated_fake)
            discriminator_loss = real_loss + fake_loss
        self.discriminator_optimizer.minimize(discriminator_loss, [self.discriminator.trainable_variables], tape=tape2)
        ...
        self.compiled_metrics.update_state(y_true=tf.ones_like(discriminated_real), y_pred=discriminated_real)
        self.compiled_metrics.update_state(y_true=tf.zeros_like(discriminated_fake), y_pred=discriminated_fake)
