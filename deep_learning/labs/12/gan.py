...
class GAN(tf.keras.Model):
    def __init__(self, args):
        ...
        for i in range(len(args.generator_layers)):
            x = tf.keras.layers.Dense(args.generator_layers[i], activation="relu")(x)
        out_dense = tf.keras.layers.Dense(MNIST.H * MNIST.W * MNIST.C, activation="sigmoid")(x)
        out_reshaped = tf.keras.layers.Reshape(target_shape=[MNIST.H, MNIST.W, MNIST.C])(out_dense)
        self.generator = tf.keras.Model(inputs=inputs, outputs=out_reshaped)
