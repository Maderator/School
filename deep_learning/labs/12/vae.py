...
class VAE(tf.keras.Model):
    def __init__(self, args):
        ...
        self.encoder = tf.keras.Model(inputs=inputs, outputs={"z_mean":z_mean, "z_log_variance":z_log_variance})
        ...
        self.decoder = tf.keras.Model(inputs=inputs, outputs=out_reshaped)
    ...
    def train_step(self, images):
        with tf.GradientTape() as tape:
            ...
            out_encoder = self.encoder(images, training=True)
            z_mean = out_encoder["z_mean"]
            z_log_variance = out_encoder["z_log_variance"]
            ...
            std = tf.exp(z_log_variance / 2)
            norm_dist = tfp.distributions.Normal(loc=z_mean, scale=std)
            z = norm_dist.sample(seed=self._seed)

            # TODO: Decode images using `z` (also passing `training=True` to the `self.decoder`).
            out_decoder = self.decoder(z, training=True)

            # TODO: Define `reconstruction_loss` using `self.compiled_loss`
            reconstruction_loss = self.compiled_loss(images, out_decoder)
            ...
            latent_loss = tf.reduce_mean(tfp.distributions.kl_divergence(norm_dist, self._z_prior))
            ...
            pixels_num = MNIST.H * MNIST.W * MNIST.C # TODO maybe delete MNIST.C
            loss = reconstruction_loss * pixels_num + latent_loss * self._z_dim
