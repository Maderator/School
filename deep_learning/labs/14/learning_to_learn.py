...
class Network(tf.keras.Model):
    ...
    class MemoryAugmentedLSTM(tf.keras.layers.AbstractRNNCell):
        ...
        def __init__(self, units, memory_cells, cell_size, read_heads, **kwargs):
            ...
            # TOD: Create the required layers:
        ...
        def state_size(self):
            ...
            heads_shape = self._read_heads
            external_memory_size = [self._memory_cells, self._cell_size]
            state_size = [controller_state_size, heads_shape, external_memory_size] # TODO probably wrong shape of heads_shape
            return state_size
        ...
        def call(self, inputs, states):
            ...
            controller_inputs = tf.concat([inputs, read_value], axis=1)
            controller_output, controller_state = self._controller(inputs=controller_inputs, states=controller_state)
            ...
            #   form the `read_keys`
            pars = self._parameters(controller_output)
            write_value = pars[:self._cell_size]

            batch_size = tf.shape(inputs)[0]
            read_keys = tf.reshape(pars[self._cell_size:], shape = [batch_size, self._read_heads, self._cell_size])
            ...
            norm_memory = [tf.math.l2_normalize(memory[0], axis=-1), tf.math.l2_normalize(memory[1], axis=-1)]
            norm_read_keys = tf.math.l2_normalize(read_keys, axis=-1)
            print(norm_memory)
            print(norm_read_keys)
            attn = tf.matmul(norm_memory[1], norm_read_keys, transpose_b=True)
            print(attn)
            attn = tf.matmul(norm_memory[0], attn) # TODO only one matmul should be used -> wrong memory dimensions
            print(attn)

            # TODO this is not complete
            #cosine_loss = tf.keras.losses.CosineSimilarity()
            #cos_sim = cosine_loss(read_keys, memory)
            #cell_distribution = tf.nn.softmax(cos_sim) # TODO choose axis
            #read_value = memory @ cell_distribution # TODO choose axis
            ...
            output = self._output_layer(tf.concat([controller_output, read_value], axis=1))
    ...
    def __init__(self, args):
        ...
        x = tf.keras.layers.TimeDistributed(tf.keras.layers.Conv2D(8, 3, 2, padding="valid"))(images)
        x = tf.keras.layers.TimeDistributed(tf.keras.layers.BatchNormalization())(x)
        x = tf.keras.layers.TimeDistributed(tf.keras.layers.Activation("relu"))(x)
        
        x = tf.keras.layers.TimeDistributed(tf.keras.layers.Conv2D(16, 3, 2, padding="valid"))(x)
        x = tf.keras.layers.TimeDistributed(tf.keras.layers.BatchNormalization())(x)
        x = tf.keras.layers.TimeDistributed(tf.keras.layers.Activation("relu"))(x)
        
        x = tf.keras.layers.TimeDistributed(tf.keras.layers.Conv2D(32, 3, 2, padding="valid"))(x)
        x = tf.keras.layers.TimeDistributed(tf.keras.layers.BatchNormalization())(x)
        x = tf.keras.layers.TimeDistributed(tf.keras.layers.Activation("relu"))(x)
        ...
        oh_previous_labels = tf.one_hot(previous_labels, depth=args.classes+1, axis=-1)
        x = tf.concat([x, oh_previous_labels], axis=2) # TODO depth maybe only args.classes (not -1:4 but 0:4)
        ...
        maLSTM = self.MemoryAugmentedLSTM(args.lstm_dim, args.classes * args.images_per_class, args.cell_size, args.read_heads)
        x = tf.keras.layers.RNN(maLSTM, return_sequences=True)(x)
        ...
        predictions = tf.keras.layers.Dense(args.classes, tf.nn.softmax)(x)
