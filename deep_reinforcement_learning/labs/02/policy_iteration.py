...
class GridWorld:
    ...
    # returns tuple of probability of action, reward, new state
    @staticmethod
...
parser.add_argument("--accuracy", default=1e-4, type=float, help="A small positive number determining the accuracy of estimation.")
    ...
    # Policy Evaluation
        for _ in range(args.iterations):
            ...
            for s in range(GridWorld.states):
                s_val = value_function[s]
                step_results = GridWorld.step(s, policy[s])
                new_value_function = 0
                for result in step_results:
                    prob, reward, new_state = result
                    new_value_function += prob * (reward + args.gamma * value_function[new_state])
                #est_accuracy = np.max(est_accuracy, np.abs(value_function[s] - new_value_function))
                value_function[s] = new_value_function
            #if est_accuracy < args.accuracy:
            #    break
