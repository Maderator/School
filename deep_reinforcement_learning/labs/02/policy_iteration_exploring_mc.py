...
def mc_simulation(start_state: int, start_action: int, env: GridWorld, policy: np.ndarray, args: argparse.Namespace) -> tuple[float]:
    rewards = np.zeros((args.mc_length))
    rewards[0], new_state = env.step(start_state, start_action)
    
    for i in range(1,args.mc_length):
        rewards[i], new_state = env.step(new_state, policy[new_state])
...
def main(args: argparse.Namespace) -> tuple[list[float], list[int]]:
    ...
    for i in range(args.steps):
        ...
        for start_state in range(env.states):
            for start_action in range(len(env.actions)):
                action_value_function[start_state, start_action] *= i/(i+1) 
                action_value_function[start_state, start_action] += mc_simulation(start_state, start_action, env, policy, args) / (i+1)
