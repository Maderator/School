...
def mc_simulation(state: int, env: GridWorld, policy: np.ndarray, args: argparse.Namespace) -> tuple[float, int]:
    rewards = np.zeros((args.mc_length))
    first_action = env.epsilon_greedy(args.epsilon, policy[state])
    rewards[0], state = env.step(state, first_action)
    for i in range(1,args.mc_length):
        greedy_action = env.epsilon_greedy(args.epsilon, policy[state])
        rewards[i], state = env.step(state, greedy_action)
    
    g = 0
    for i in range(args.mc_length-1,-1,-1):
        g = args.gamma * g + rewards[i]
    return (g, first_action)

def get_best_value_function(action_val_func: np.ndarray, policy: np.ndarray):
    states = np.linspace(0, len(policy), num=len(policy), endpoint=False, dtype=np.int32)
    return action_val_func[states, policy]
...
def main(args: argparse.Namespace) -> tuple[list[float], list[int]]:
    ...
    action_visits_count = np.zeros((env.states, len(env.actions)))
    
    for i in range(args.steps):
        # Policy Evaluation
        for start_state in range(env.states):
            g, first_action = mc_simulation(start_state, env, policy, args)
            action_visits_count[start_state, first_action] += 1
            c = action_visits_count[start_state, first_action]
            q = action_value_function[start_state, first_action]
            action_value_function[start_state, first_action] += 1/c * (g-q)
        # Policy Improvement
    ...
    value_function = get_best_value_function(action_value_function, policy)
