...
parser.add_argument("--verbose", default=False, action="store_true", help="More verbose output.")
...
def main(args: argparse.Namespace) -> tuple[list[float], list[int]]:
    ...
    for _ in range(args.steps):
        # Policy evaluation
        T = np.zeros([GridWorld.states, GridWorld.states])
        R = np.zeros(GridWorld.states)
        for s in range(GridWorld.states):
            for probability, reward, state in GridWorld.step(s, policy[s]):
                T[s, state] += probability
                R[s] += reward * probability
        value_function = np.linalg.solve(np.eye(GridWorld.states) - args.gamma * T, R)

        # Policy improvement
        for s in range(GridWorld.states):
            values = [0] * len(GridWorld.actions)
            for a in range(len(GridWorld.actions)):
                for probability, reward, state in GridWorld.step(s, a):
                    values[a] += probability * (reward + args.gamma * value_function[state])
            policy[s] = argmax_with_tolerance(values)
