...
def main(args: argparse.Namespace) -> tuple[list[float], list[int]]:
    ...
    for step in range(args.steps):
        ...
        for start_state in range(env.states):
            for start_action in range(len(env.actions)):
                state, action, rewards = start_state, start_action, 0
                for i in range(args.mc_length):
                    reward, state = env.step(state, action)
                    rewards += reward * args.gamma ** i
                    action = policy[state]
                action_value_function[start_state, start_action] += 1 / (step + 1) * (rewards - action_value_function[start_state, start_action])
        ...
        if args.verbose:
            for l in range(3):
                for c in range(4):
                    state = l * 4 + c
                    if state >= 5: state -= 1
                    print("        " if l == 1 and c == 1 else "{:-8.2f}".format(action_value_function[state, policy[state]]), end="")
