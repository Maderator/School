...
def main(args: argparse.Namespace) -> tuple[list[float], list[int]]:
    ...
    visits = np.zeros((env.states, len(env.actions)), np.int32)
    ...
    for step in range(args.steps):
        ...
        for state in range(env.states):
            ...
            for i in range(args.mc_length):
                ...
                if i == 0: start = (state, action)
                reward, state = env.step(state, action)
                rewards += reward * args.gamma ** i
            visits[start] += 1
            action_value_function[start] += 1 / visits[start] * (rewards - action_value_function[start])
