...
parser.add_argument("--n", default=8, type=int, help="N-step method.")
    ...
    while training:
        ...
        while not done:
            ...
            while len(states) >= (args.n if not done else 1):
                G = reward + args.gamma * np.max(W[next_state].sum(axis=0))
                for i in reversed(range(1, len(states))):
                    greedy_action = np.argmax(W[states[i]].sum(axis=0))
                    G = rewards[i-1] + args.gamma * (G if actions[i] == greedy_action else W[states[i], greedy_action].sum())
