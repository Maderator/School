...
class Network:
    def __init__(self, env: wrappers.EvaluationEnv, args: argparse.Namespace) -> None:
        ...
        self._model = tf.keras.Sequential([
            ...
            tf.keras.layers.Dense(env.action_space.n, activation=None),
        ])
    ...
    def train(self, states: np.ndarray, actions: np.ndarray, q_values: np.ndarray) -> None:
        self._model.optimizer.minimize(
            lambda: self._model.compiled_loss(
                q_values,
                tf.gather(self._model(states, training=True), actions, batch_dims=1),
            ),
    ...
    while training:
        ...
        while not done:
            ...
            if len(replay_buffer) >= args.batch_size:
                batch = [replay_buffer[i] for i in np.random.choice(len(replay_buffer), size=args.batch_size, replace=False)]
                states, actions, rewards, dones, next_states = map(np.array, zip(*batch))

                targets = rewards + args.gamma * (1 - dones) * np.max(target.predict(next_states), axis=1)
                network.train(states, actions, targets)
