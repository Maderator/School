...
class Network:
    def __init__(self, env: wrappers.EvaluationEnv, args: argparse.Namespace) -> None:
        ...
        # Actor
        states = tf.keras.layers.Input(shape=env.observation_space.shape)
        hidden = tf.keras.layers.Dense(args.hidden_layer_size, activation=tf.nn.relu)(states)
        hidden = tf.keras.layers.Dense(args.hidden_layer_size, activation=tf.nn.relu)(hidden)
        actions = tf.keras.layers.Dense(env.action_space.shape[0], activation=tf.nn.tanh)(hidden)
        actions = (env.action_space.high - env.action_space.low) / 2 * actions + (env.action_space.high + env.action_space.low) / 2
        self._actor = tf.keras.Model(inputs=states, outputs=actions)
        ...
        self._critic = tf.keras.Model(inputs=[states, actions], outputs=returns)
        self._target_critic = tf.keras.models.clone_model(self._critic)
        self._critic.compile(tf.keras.optimizers.Adam(args.learning_rate))
    ...
    def train(self, states: np.ndarray, actions: np.ndarray, returns: np.ndarray) -> None:
        ...
        self._actor.optimizer.minimize(
            lambda: -tf.math.reduce_mean(self._critic([states, self._actor(states, training=True)])),
        ...
        self._critic.optimizer.minimize(
            lambda: tf.keras.losses.mean_squared_error(returns, self._critic([states, actions], training=True)),
