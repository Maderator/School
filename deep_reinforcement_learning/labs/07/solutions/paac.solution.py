...
class Network:
    def __init__(self, env: wrappers.EvaluationEnv, args: argparse.Namespace) -> None:
        ...
        self._critic = tf.keras.Sequential([
            tf.keras.layers.Dense(args.hidden_layer_size, activation=tf.nn.relu),
    ...
    def train(self, states: np.ndarray, actions: np.ndarray, returns: np.ndarray) -> None:
        ...
        baseline = self._critic(states)[:, 0]
        self._actor.optimizer.minimize(
            lambda: self._actor.loss(actions, self._actor(states, training=True), returns - baseline),
            var_list=self._actor.trainable_variables
        )
        self._critic.optimizer.minimize(
            lambda: self._critic.loss(returns, self._critic(states, training=True)[:, 0]),
...
def main(env: wrappers.EvaluationEnv, args: argparse.Namespace) -> None:
    ...
    while training:
        ...
        for _ in range(args.evaluate_each):
            ...
            actions = []
            for probabilities in network.predict_actions(states):
                actions.append(np.random.choice(env.action_space.n, p=probabilities))
