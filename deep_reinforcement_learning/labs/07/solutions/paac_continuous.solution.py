...
parser.add_argument("--hidden_layer_size", default=30, type=int, help="Size of hidden layer.")
...
class Network:
    def __init__(self, env: wrappers.EvaluationEnv, args: argparse.Namespace) -> None:
        ...
        states = tf.keras.layers.Input(shape=env.observation_space.shape, dtype=tf.int32)
        tiles = tf.reduce_sum(tf.one_hot(states, env.observation_space.nvec[-1]), axis=1)

        # If we wanted to use embeddings, we could use the following instead of the
        # first line after the comment:
        #    hidden = tf.keras.layers.Embedding(env.observation_space.nvec[-1], args.hidden_layer_size)(states)
        #    hidden = tf.nn.relu(tf.math.reduce_sum(hidden, axis=1))
        hidden = tf.keras.layers.Dense(args.hidden_layer_size, activation=tf.nn.relu)(tiles)
        mus = tf.keras.layers.Dense(env.action_space.shape[0], activation=tf.nn.tanh)(hidden)
        mus = (env.action_space.high - env.action_space.low) / 2 * mus + (env.action_space.high + env.action_space.low) / 2

        hidden = tf.keras.layers.Dense(args.hidden_layer_size, activation=tf.nn.relu)(tiles)
        sds = tf.keras.layers.Dense(env.action_space.shape[0], activation=tf.nn.softplus)(hidden)

        hidden = tf.keras.layers.Dense(args.hidden_layer_size, activation=tf.nn.relu)(tiles)
        values = tf.keras.layers.Dense(1)(hidden)[:, 0]

        self._model = tf.keras.Model(inputs=states, outputs=[mus, sds, values])
    ...
    def train(self, states: np.ndarray, actions: np.ndarray, returns: np.ndarray) -> None:
        ...
        with tf.GradientTape() as tape:
            ...
            loss = tf.math.reduce_mean(tf.math.reduce_sum(-action_distribution.log_prob(actions), axis=1) * (returns - tf.stop_gradient(values)))
            loss += -self._entropy_regularization * tf.math.reduce_mean(action_distribution.entropy())
            loss += tf.losses.mean_squared_error(returns, values)
...
def main(env: wrappers.EvaluationEnv, args: argparse.Namespace) -> None:
    ...
    while training:
        ...
        for _ in range(args.evaluate_each):
            ...
            actions = []
            for mus, sds in zip(*network.predict_actions(states)):
                actions.append(np.clip(np.random.normal(mus, sds), env.action_space.low, env.action_space.high))
