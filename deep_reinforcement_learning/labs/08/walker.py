...
parser.add_argument("--hidden_layer_size", default=500, type=int, help="Size of hidden layer.")
...
parser.add_argument("--target_return", default=200, type=int, help="Needed score for training to end.")


#def get_adamw_optimizer(args):
...
class Network:
    def __init__(self, env: wrappers.EvaluationEnv, args: argparse.Namespace) -> None:
        ...
        class Actor(tf.keras.Model):
            def __init__(self, hidden_layer_size: int, env: wrappers.EvaluationEnv):
                ...
                self._log_alpha = tf.Variable(np.log(0.1), dtype=tf.float32, trainable=True)
                self.input_layer = tf.keras.layers.InputLayer(input_shape=env.observation_space.shape)
                self.dense1 = tf.keras.layers.Dense(
                    hidden_layer_size, activation=tf.keras.activations.relu)
                self.dense2 = tf.keras.layers.Dense(
                    hidden_layer_size, activation=tf.keras.activations.relu)
                self.mus_out = tf.keras.layers.Dense(env.action_space.shape[0], activation=None)
                self.sds_out = tf.keras.layers.Dense(env.action_space.shape[0], activation=tf.math.exp)

            def get_train_vars_no_alpha(self):
                return (self.dense1.trainable_variables + 
                    self.dense2.trainable_variables +
                    self.mus_out.trainable_variables + 
                    self.sds_out.trainable_variables)
            ...
            def call(self, inputs: tf.Tensor, sample: bool):
                ...
                ins = self.input_layer(inputs)
                d1_out = self.dense1(ins)
                d2_out = self.dense2(d1_out)
                mus = self.mus_out(d2_out)
                sds = self.sds_out(d2_out)
                if sample:
                    actions_distribution = tfp.distributions.Normal(loc=mus, scale=sds)
                else:
                    actions_distribution = tfp.distributions.Normal(loc=mus, scale=tf.zeros(sds.shape))
                ...
                log_probs = tf.math.reduce_mean(actions_distribution.log_prob(actions), axis=-1, keepdims=True)
        ...
        self._alpha_optimizer = tf.keras.optimizers.Adam(learning_rate=args.learning_rate)
        ...
        in_observations = tf.keras.layers.Input(shape=env.observation_space.shape)
        in_actions = tf.keras.layers.Input(shape=env.action_space.shape)
        concat_inputs = tf.keras.layers.Concatenate()([in_observations, in_actions])
        hidden = tf.keras.layers.Dense(
            args.hidden_layer_size, activation=tf.keras.activations.relu)(concat_inputs)
        hidden = tf.keras.layers.Dense(
            args.hidden_layer_size, activation=tf.keras.activations.relu)(hidden)
        output = tf.keras.layers.Dense(1, activation=None)(hidden)

        self._critic1 = tf.keras.Model([in_observations, in_actions], output)
        self._critic1.compile(
            optimizer=tf.optimizers.Adam(learning_rate=args.learning_rate),
            # TODO use AdamW
            loss=tf.losses.MeanSquaredError()
        )
        self._critic2 =         tf.keras.models.clone_model(self._critic1)
        ...
        self._target_critic1 =  tf.keras.models.clone_model(self._critic1)
        self._target_critic2 =  tf.keras.models.clone_model(self._critic1)
    ...
    def train(self, states: np.ndarray, actions: np.ndarray, returns: np.ndarray) -> None:
        ...
        with tf.GradientTape() as tape:
            pred_actions, log_prob, alpha = self._actor(states, sample=True)
            loss = tf.stop_gradient(alpha) * log_prob
            critic_input = [states, pred_actions]
            pred_returns1 = self._target_critic1(critic_input) - tf.stop_gradient(alpha) * log_prob
            pred_returns2 = self._target_critic2(critic_input) - tf.stop_gradient(alpha) * log_prob
            pred_returns = tf.math.minimum(pred_returns1, pred_returns2)
            loss = loss - (pred_returns-returns)
            loss = tf.math.reduce_mean(loss)
        #self._actor.optimizer.minimize(loss, self._actor.get_train_vars_no_alpha(), tape=tape)
        self._actor.optimizer.minimize(loss, self._actor.trainable_variables, tape=tape)
        
        with tf.GradientTape() as tape:
            pred_actions, log_prob, alpha = self._actor(states, sample=True)
            loss = tf.math.reduce_mean(- alpha * tf.stop_gradient(log_prob) - alpha * args.target_entropy)
        #self._alpha_optimizer.minimize(loss, [self._actor._log_alpha], tape=tape)
        self._alpha_optimizer.minimize(loss, self._actor.trainable_variables, tape=tape)

        with tf.GradientTape() as tape:
            value_pred = self._critic1([states, actions], training=True)
            loss = self._critic1.compiled_loss(y_true=returns, y_pred=value_pred)
        self._critic1.optimizer.minimize(loss, self._critic1.trainable_variables, tape=tape)
        
        with tf.GradientTape() as tape:
            value_pred = self._critic2([states, actions], training=True)
            loss = self._critic2.compiled_loss(y_true=returns, y_pred=value_pred)
        self._critic2.optimizer.minimize(loss, self._critic2.trainable_variables, tape=tape)
    ...
    def predict_values(self, states: np.ndarray) -> np.ndarray:
        ...
        pred_returns1 = self._target_critic1(critic_input) - alpha * log_prob
        pred_returns2 = self._target_critic2(critic_input) - alpha * log_prob
        return tf.math.minimum(pred_returns1, pred_returns2)
    ...
    while training:
        for _ in range(args.evaluate_each):
            ...
            if len(replay_buffer) >= 4 * args.batch_size:
                ...
                # TODO: Perform the training (zatim zkopirovane z ddpg)
        ...
        train_mean = np.mean(env._episode_returns[-min(args.evaluate_each, 100):])
        if train_mean > best_mean:
            best_mean = train_mean
            print(f"new best mean {best_mean}... saving actor")
