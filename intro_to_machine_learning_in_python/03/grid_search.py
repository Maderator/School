...
def main(args):
    ...
    model = sklearn.pipeline.Pipeline([('preprocessor', minmax),
                                    ('features', pf),
                                    ('classificator', clf)])
   
    skf = sklearn.model_selection.StratifiedKFold(5)

    param_grid={'classificator__C' : [0.01, 1, 100],
                'classificator__solver' : ('lbfgs','sag'),
    ...
    test_accuracy = sum(pred == test_target) / test_target.shape[0]
