...
def main(args):
    ...
    one_feature = np.ones((data.shape[0],1))
    data = np.concatenate((data, one_feature), axis=1)
    ...
    train_data, test_data, train_target, test_target = sklearn.model_selection.train_test_split(data, 
        target, test_size=args.test_size, random_state=args.seed)
    ...
    for iteration in range(args.iterations):
        ...
        perm_data, perm_target = train_data[permutation], train_target[permutation]
        batches = round(train_data.shape[0]/args.batch_size)
        for b in range(batches):
            first = b*args.batch_size
            last = (b+1)*args.batch_size
            batch = perm_data[first : last]
            batch_target = perm_target[first:last]
            cond_probs = expit(batch @ weights)
            grads = (batch_target-cond_probs)*batch.T
            avg_grad = - 1/args.batch_size * grads.sum(axis=1)
        ...
        # see the 4th slide: http://sgunasekar.github.io/website-intromlss2018/course_material/Day3b.pdf
        train_loss = 1/train_data.shape[0] * sum(-train_target * np.log(expit(train_data @ weights)) - (1-train_target) * np.log(expit(-train_data @ weights)))
        test_loss = 1/test_data.shape[0] * sum(-test_target * np.log(expit(test_data @ weights)) - (1-test_target) * np.log(expit(-test_data @ weights)))

        train_pred = train_data @ weights
        test_pred = test_data @ weights
        train_pred = (train_pred > 0).astype(int)
