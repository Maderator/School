...
def test_insert(a, b):
    print("test insert a={}, b={}".format(a, b))
    filename = "insert{}_{}.csv".format(a,b)
    with open(filename, 'w') as csvfile:
        ...
        for exp in range(32, 64):
            ...
            writer.writerow([n] + [tree.struct_changes_per_op()])
            print(n, tree.struct_changes_per_op())

def test_random(a, b):
    print("test random a={}, b={}".format(a, b))
    filename = "random{}_{}.csv".format(a,b)
    with open(filename, 'w') as csvfile:
        writer = csv.writer(csvfile, delimiter=',')
        for exp in range(32, 64):
            n = int(2**(exp/4))
...
def test_min(a, b):
    print("test min a={}, b={}".format(a, b))
    filename = "min{}_{}.csv".format(a,b)
    with open(filename, 'w') as csvfile:
        writer = csv.writer(csvfile, delimiter=',')
        for exp in range(32, 64):
            n = int(2 ** (exp / 4))
...
def do_tests(tests_names, student_id, n_args):
    a = 2
    if sys.argv[n_args-1] == "2-3":
        b = 3
    elif sys.argv[n_args-1] == "2-4":
        b = 4
    else:
...
if __name__ == '__main__':
    ...
    if len(sys.argv) == 3:
        ...
        do_tests(tests_names, student_id, 3)
    elif len(sys.argv) == 4:
        test, student_id = sys.argv[1], sys.argv[2]
        do_tests([test], student_id, 4)
    else:
        raise ValueError("Usage: {} <test> <student-id> (2-3|2-4) \n \
            or {} <student-id> (2-3|2-4)".format(sys.argv[0], sys.argv[0]))
